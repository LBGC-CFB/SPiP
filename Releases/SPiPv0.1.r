...
if(libError=1){stop("******Library error*******")}
...
#get script argument
option_list = list(make_option(opt_str = c("-F","--Input"), action="store", type="character", default=NULL,
						help="list of variants file (.txt or .vcf)", metavar="character"),
					make_option(c("-O",'--Output'), type="character", default=NULL,
						help="Name of ouput file (.txt)", metavar="character"),
					make_option(c("-g",'--GenomeAssenbly'), type="character", default="hg19",
						help="Genome assembly version (hg19 of hg38) [default= %default]", metavar="character"),
					make_option(c("-s",'--SamPath'), type="character", default=NULL,
						help="Path to samtools, if you want to use Ensembl api keep this argument empty", metavar="character"),
					make_option(c("-f",'--fastaGenome'), type="character", default=NULL,
						help="fasta file of genome used by samtools", metavar="character"),
					make_option(c("-c",'--SPiCE'), type="character", default="Yes",
						help="Use of consensus prediction (SPiCE) (Yes/No) [default= %default]", metavar="character"),
					make_option(c("-m",'--MES'), type="character", default="Yes",
						help="Use of MES prediction in {-20; -13} (Yes/No) [default= %default]", metavar="character"),
					make_option(c("-b",'--BP'), type="character", default="Yes",
						help="Use of Branch point prediction (Yes/No) [default= %default]", metavar="character"),
					make_option(c("-d",'--Cryptic'), type="character", default="Yes",
						help="Use of de Novo/cryptic prediction (Yes/No) [default= %default]", metavar="character"),
					make_option(c("-e",'--ESR'), type="character", default="Yes",
						help="Use of ESR prediction (Yes/No) [default= %default]", metavar="character")
)
...
if(is.null(opt$Input)|is.null(opt$Output)){
	message("###########################")
	message("#No input or output file")
	message("###########################")
...
if(is.null(opt$SamPath)){
	useEnsemblAPI="YES"
}else{
	useEnsemblAPI="NO"
	samPath = opt$SamPath
	if(is.null(opt$fastaGenome)){
		message("###########################")
		...
		fastaFile = opt$fastaGenome
	}
}

if(!is.null(opt$SPiCE)){
	useOption["SPiCE"]=opt$SPiCE
}
if(!is.null(opt$MES)){
	useOption["MES"]=opt$MES
}
if(!is.null(opt$BP)){
	useOption["BP"]=opt$BP
}
if(!is.null(opt$Cryptic)){
	useOption["Cryptic"]=opt$Cryptic
}
if(!is.null(opt$ESR)){
	useOption["ESR"]=opt$ESR
}
...
data$ESRMut <- NA
data$probaMut <- NA
data$classProbaMut <- NA
data$posWT <- NA
data$MESWT <- NA
data$SSFWT <- NA
data$ESRWT <- NA
data$probaWT <- NA
data$classProbaWT <- NA
data$posPhysio <- NA
data$MESPhysio <- NA
data$SSFPhysio <- NA
data$ESRPhysio <- NA
data$probaPhysio <- NA
data$classProbaPhysio <- NA
data$probaPhysioMut <- NA
data$classProbaPhysioMut <- NA
...
getVariantInfo <- function(varID){
	...
	if(length(varDecomp)!=3 & length(varDecomp)!=2){
		tkmessageBox(message = "You must import variant as:\nTranscrit:position(:)nucleotidic change", icon = "error", type = "ok")
	}else{
...
getGlobaInterpretation <- function(SPiCEinterpret, RegType, deltaMES, mutInPBareaBPP, classProbaMut, distSS, deltaESR,useOption){
	interpretFinal = ""
	if((SPiCEinterpret=="high" | SPiCEinterpret=="medium") & useOption["SPiCE"]=="Yes"){
		interpretFinal = "Alter by SPiCE"
	}else if (length(grep("PolyTC",RegType ))>0 & deltaMES<(-0.15) & useOption["MES"]=="Yes"){
		interpretFinal = "Alter by MES (Poly TC)"
	}else if (length(grep("BP",RegType ))>0 & mutInPBareaBPP!="No" & useOption["BP"]=="Yes"){
		interpretFinal = "Alter BP"
	}else if (classProbaMut=="Yes" & useOption["Cryptic"]=="Yes"){
		if(abs(distSS)>=150 & length(grep("Intron",RegType ))>0) {
			interpretFinal = "Alter by create Exon"
		}else{
			interpretFinal = "Alter by create Cryptic"
		}
	}else if (length(grep("Exon",RegType ))>0 & abs(distSS)<120){
		if(deltaESR<(-1.25) & useOption["ESR"]=="Yes") {
			interpretFinal = "Alter ESR"
		}else{
			interpretFinal = "NTR ESR"
		}
	}else{
		interpretFinal = "NTR"
	}
...
getOutput <- function(data,i){
	...
	if(nrow(tmpTableSeqNoPhyMut)>0){
		...
		data$posMut[i] <<- tmpTableSeqMAXMut$pos
		data$sstype[i] <<- as.character(tmpTableSeqMAXMut$sstype)
		...
		data$MESMut[i] <<- tmpTableSeqMAXMut$MES
		data$SSFMut[i] <<- tmpTableSeqMAXMut$SSF
		data$ESRMut[i] <<- tmpTableSeqMAXMut$ESR
		data$probaMut[i] <<- tmpTableSeqMAXMut$proba
		data$classProbaMut[i] <<- tmpTableSeqMAXMut$classProba
		if(nrow(tmpTableSeqMAXWT)>0){
			data$posWT[i] <<- tmpTableSeqMAXWT$pos[1]
			data$MESWT[i] <<- tmpTableSeqMAXWT$MES[1]
			data$SSFWT[i] <<- tmpTableSeqMAXWT$SSF[1]
			data$ESRWT[i] <<- tmpTableSeqMAXWT$ESR[1]
			data$probaWT[i] <<- tmpTableSeqMAXWT$proba[1]
			data$classProbaWT[i] <<- tmpTableSeqMAXWT$classProba[1]
		}
		if(nrow(tmpTableSeqPhy)>0){
			data$posPhysio[i] <<- tmpTableSeqPhy$pos[1]
			data$MESPhysio[i] <<- tmpTableSeqPhy$MES[1]
			data$SSFPhysio[i] <<- tmpTableSeqPhy$SSF[1]
			data$ESRPhysio[i] <<- tmpTableSeqPhy$ESR[1]
			data$probaPhysio[i] <<- tmpTableSeqPhy$proba[1]
			data$classProbaPhysio[i] <<- tmpTableSeqPhy$classProba[1]
			data$probaPhysioMut[i] <<- tmpTableSeqPhy$proba[1]
			data$classProbaPhysioMut[i] <<- tmpTableSeqPhy$classProba[1]
		}
		if(nrow(tmpTableSeqPhyMut)>0){
			data$probaPhysioMut[i] <<- tmpTableSeqPhyMut$proba[1]
			data$classProbaPhysioMut[i] <<- tmpTableSeqPhyMut$classProba[1]
		}
	}else{
		data$posMut[i] <<- 0
		data$sstype[i] <<- "No site"
		...
		data$ESRMut[i] <<- 0
		data$probaMut[i] <<- 0
		data$classProbaMut[i] <<- "No"
		data$posWT[i] <<- 0
		data$MESWT[i] <<- 0
		data$SSFWT[i] <<- 0
		data$ESRWT[i] <<- 0
		data$probaWT[i] <<- 0
		data$classProbaWT[i] <<- "No"
		data$posPhysio[i] <<- 0
		data$MESPhysio[i] <<- 0
		data$SSFPhysio[i] <<- 0
		data$ESRPhysio[i] <<- 0
		data$probaPhysio[i] <<- 0
		data$classProbaPhysio[i] <<- "No"
		data$probaPhysioMut[i] <<- 0
		data$classProbaPhysioMut[i] <<- "No"
	}
	data$Interpretation[i] <<- getGlobaInterpretation(SPiCEinter_2thr, RegType, deltaMES, mutInPBareaBPP, data$classProbaMut[i], distSS[1], ESRscore,useOption)
}
